 ...
 * O teste `tests/semCoercaoDeDistancia.test.js` procura a coerção no código.
 */
...
/** Nomes de município iguais, sem acento, caixa ou espaço extra. */
...
/**
 * Referência residencial resolvida só em nível de município (CEP genérico,
 * logradouro não geocodificado). O ponto é um centroide ou a sede, não a casa.
 *
 * ─── O defeito que motivou ───────────────────────────────────────────────────
 * Laudo FD-20260922-ABC18B73E5: domicílio "Manaquiri, AM" resolvido pelo CEP
 * 69435-000 no centroide do município, 55 km ao sul da sede. O GPS do ato caía
 * em Bacabal, dentro da sede. O laudo chamou de "divergência grave, 69,6 km" a
 * própria cidade da cliente, e o índice 37/100 nasceu disso.
 */
...
export function montarConfrontoGeografico(result = {}) {
  ...
  // Referência em nível de município: ponto do ato no mesmo município não gera
  // distância residencial. O confronto válido é o de município, e ele coincide.
